...
static set_t alloc_set_inner(unsigned int properties, int sz);
...
set_t alloc_set(unsigned int properties) { return alloc_set_inner(properties, INITIAL_SIZE); }

static set_t alloc_set_inner(unsigned int properties, int c)
{
  ...
  if (properties & SP_MAP) sz += c * sizeof(void*);
...
void 
set_header_set(set_t s, void* header)
{
  set_head_t *sh = get_set_head(s);
